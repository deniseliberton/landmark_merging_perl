...
# It starts looking for landmarks on line 9 of the input file. If needed, change line 42 of this script to the appropriate line in the input file.
...
open (OFILE, ">$outfile");
#look only for files that end in .pp
@file = <*.pp>;
...
# skip all lines before the ninth one (where the coordinates start)
		next if $. < 9; 
# skip all lines that don't begin with "<point"
		if (m/^\s[<point]/) {
		$line = $_;
# removes the newline/end of line character from each line		
		chomp($line);		
